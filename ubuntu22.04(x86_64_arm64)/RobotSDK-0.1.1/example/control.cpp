...
std::map<char, CommandHandler> CreateCommandTable(SDKClient& sdk_client) {
  return {
      ...
      {'w', [](SDKClient& client) { client.Move(0.0, 0.2, 0.0); }},  // Forward
      ...
      // ============ Turn Control ============
      {'l',
       [](SDKClient& client) { client.Move(0.0, 0.0, 0.05); }},  // Left turn
      {'r',
       [](SDKClient& client) { client.Move(0.0, 0.0, -0.05); }},  // Right turn
